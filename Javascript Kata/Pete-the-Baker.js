...
function cakes(recipe, available) {
  var items = [];
  var rkeys = Object.keys(recipe).sort();
  var akeys = Object.keys(available).sort();
  for (var i = 0; i < rkeys.length; i++){
    if (!akeys.includes(rkeys[i])) {
      return 0;
    } else {
      var num = Math.floor(available[rkeys[i]] / recipe[rkeys[i]])
      items.push(num)
    }
  }
  return items.sort((a, b) => a - b)[0]
}
